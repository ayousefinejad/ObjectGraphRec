#!/usr/bin/env bash
# Seed confirmation for rho=2, both arms. The seed-0 sweep put rho=2 above the published rho=1
# by +0.0022 (+object) and +0.0026 (image+text) -- roughly 2x the 0.0012 resolution band, but on
# one seed. This result currently contradicts RESULTS.md section 7.1, which concluded the CF
# backbone was the dominant axis while holding rho fixed at its untuned default of 1, so it needs
# replicates before either statement can stand.
#
# Original sweep header follows.
...
for SEED in 1 2; do
  run_one 2 obj   "$SEED"                          || exit 1
  run_one 2 noobj "$SEED" --modalities image,text  || exit 1
done
