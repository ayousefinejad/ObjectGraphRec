...
# Pool size only controls how the per-user ranking is parallelised -- results are identical
# whatever this is set to. The default keeps the original cpu_count()//5; LATTICE_EVAL_CORES
# raises it for the encoder sweep, where evaluation (not training) dominates wall clock.
cores = int(os.environ.get("LATTICE_EVAL_CORES", multiprocessing.cpu_count() // 5))
