...
def parse_args():
    ...
    # The object modality. Left out of --modalities, none of the object code constructs and the
    # model is the published two-modality MICRO. The default therefore reproduces the published
    # runs bit-identically: the graph modules are built last, so they cannot shift the init RNG
    # stream of anything else.
    parser.add_argument('--modalities', nargs='?', default='image,text',
                        help='Comma-separated subset of {image, text, graph} to fuse')

    # Which of the two pathways the graph modality is allowed to act through. MICRO feeds every
    # modality into both the softmax fusion and its own InfoNCE term, so a modality that hurts has
    # two ways to do it; these arms separate them. Ignored unless 'graph' is in --modalities.
    parser.add_argument('--graph_mode', nargs='?', default='both',
                        choices=['both', 'fusion', 'contrast'],
                        help='Graph modality pathway: both, fusion only, or contrastive only')
