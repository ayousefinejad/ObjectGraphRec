...
class Trainer(object):
    def __init__(self, data_config):
        ...
        graph_feats = None
        if 'graph' in [m.strip() for m in args.modalities.split(',')]:
            graph_feats = np.load('../data/{}/object_feat.npy'.format(args.dataset))

        self.model = MICRO(self.n_users, self.n_items, self.emb_dim, self.weight_size, self.mess_dropout, image_feats, text_feats, graph_feats)
    ...
    def train(self):
        ...
        for epoch in (range(args.epoch)):
            ...
            for idx in (range(n_batch)):
                ...
                ua_embeddings, ia_embeddings, contrast_embeds, fusion_embed = self.model(self.norm_adj, build_item_graph=build_item_graph)
                ...
                for modal_embed in contrast_embeds:
                    batch_contrastive_loss += self.model.batched_contrastive_loss(modal_embed, fusion_embed)
